...
final class UsageViewModel: ObservableObject {
    ...
    private enum Keys {
        static let notificationsEnabled = "claudeusage.notificationsEnabled"
    }

    init(useMock: Bool = false) {
        let store = SessionStore()
        self.sessionStore = store
        self.client = useMock ? MockUsageClient() : ClaudeUsageClient(sessionStore: store)
    ...
    private func checkThresholds(_ data: UsageData) {
        ...
        if let opus = data.opus {
            windows.append(("Opus", opus.utilization))
        }
...
enum AlertLevel {
    ...
    var symbolName: String {
        switch self {
        case .ok:       return "brain.head.profile"
